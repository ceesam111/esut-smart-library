...
import { completeEmailVerification } from '@/server/registration/completeEmailVerification';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (getBearerToken(request)) {
      ...
      // Without a session the caller must prove it owns the account: a signed
      // receipt from /api/registration/prove-password (password proof).
      const receipt = typeof body.recoveryReceipt === 'string' ? body.recoveryReceipt : null;
      if (!userId || !receipt || !verifyRecoveryReceipt(receipt, userId)) {
        throw new Error('Authentication or registration context required.');
      }
    }
