...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('agent_job_templates').select('*').eq('tenant_id', tenant.tenantId).order('created_at', { ascending: false });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const supabase = getSupabaseAdminClient();
    const { data, error } = await supabase.from('agent_job_templates').insert({
