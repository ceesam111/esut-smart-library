...
export default function MyProfile() {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate file size: must be between 50 KB and 240 KB
    const minBytes = 50 * 1024;   // 50 KB
    const maxBytes = 240 * 1024;  // 240 KB
    if (file.size < minBytes) {
      alert(`Photo is too small (${Math.round(file.size / 1024)} KB). Minimum size is 50 KB. Please use a higher-quality image.`);
      e.target.value = '';
      return;
    }
    if (file.size > maxBytes) {
      alert(`Photo is too large (${Math.round(file.size / 1024)} KB). Maximum size is 240 KB. Please compress the image before uploading.`);
      e.target.value = '';
      return;
    }
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage
        .from('profile-photos')
        .upload(`researcher/${fileName}`, file, { upsert: true });

      if (uploadError) throw uploadError;

      const { data } = supabase.storage
        .from('profile-photos')
        .getPublicUrl(`researcher/${fileName}`);

      setProfile((prev) =>
        prev ? { ...prev, profile_photo_url: data.publicUrl } : null
      );
    } catch (error) {
