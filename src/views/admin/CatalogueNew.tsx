...
interface SimpleForm {
  ...
  call_number: string; abstract: string; series: string; physical_description: string;
...
export default function CatalogueNew() {
  ...
  const [form, setForm] = useState<SimpleForm>({
    ...
    call_number: '', abstract: '', series: '', physical_description: '', notes: '', cover_image: '',
  });
    ...
    try {
      ...
      if (olBook) {
        applyBookData({
          ...
          year: olBook.publish_date ? parseInt(olBook.publish_date) || new Date().getFullYear() : new Date().getFullYear(),
          subjects: olBook.subjects?.map((s: any) => s.name ?? s).join('; ') ?? '',
          cover_image: olBook.cover?.large ?? olBook.cover?.medium ?? '',
          physical_description: olBook.physical_format ? `${olBook.number_of_pages ?? ''} p. ; ${olBook.physical_format}` : '',
        });
        setIsbnSource('Open Library');
        return;
      }
      ...
      if (gbItem) {
        applyBookData({
          ...
          year: gbItem.publishedDate ? parseInt(gbItem.publishedDate) : new Date().getFullYear(),
          subjects: gbItem.categories?.join('; ') ?? '',
          cover_image: gbItem.imageLinks?.thumbnail?.replace('http:', 'https:') ?? '',
          physical_description: gbItem.pageCount ? `${gbItem.pageCount} pages` : '',
          abstract: gbItem.description ?? '',
        });
        setIsbnSource('Google Books');
      } else {
        setErrorMsg('No data found for this ISBN. Please fill in manually.');
  ...
  const suggestSubjects = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/ai-librarian`, {
        method: 'POST',
  ...
  const importZ3950Record = (record: any) => {
    applyBookData({
      ...
      year: record.publish_date ? parseInt(record.publish_date) || new Date().getFullYear() : new Date().getFullYear(),
    ...
    try {
      ...
      if (mode === 'simple') {
        ...
        payload = {
          ...
          year: form.year, edition: form.edition || null, subjects: subjectsArr,
          ...
          call_number: form.call_number || null, abstract: form.abstract || null,
        ...
        payload = {
          ...
          year: parseInt(year264c) || null, edition: getMarc('250', 'a') || null,
          subjects: subjects650, format: form.format, language: form.language,
          total_copies: form.copies, available_copies: form.copies,
          faculty_code: form.faculty_code || null, call_number: form.call_number || null,
  ...
  return (
    <div className="p-8 max-w-5xl space-y-6">
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-neutral-800">Add Catalogue Item</h1>
          <p className="text-neutral-500 mt-1">Create a new bibliographic record</p>
        </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={autoFillISBN} disabled={isbnLoading || !form.isbn.trim()} className="btn-primary disabled:opacity-50 whitespace-nowrap">
                  {isbnLoading ? <Spinner /> : 'Auto-Fill'}
                </button>
              </div>
              <p className="text-xs text-neutral-400 mt-1">Fills from Open Library → Google Books</p>
            </div>
            <div>
              <label className="label">Format</label>
              <select className="input" value={form.format} onChange={e => setForm(p => ({ ...p, format: e.target.value }))}>
                {['Book', 'E-Book', 'Journal', 'Periodical', 'Thesis', 'Report', 'Map', 'Video', 'Audio', 'Dataset'].map(f => <option key={f}>{f}</option>)}
              </select>
            </div>
          </div>
