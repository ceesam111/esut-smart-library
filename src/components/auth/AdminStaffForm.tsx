...
import { GENDERS, PREFERRED_BRANCHES } from '@/config/roles.config';
import { institutionConfig } from '@config/institution.config';
import { registerAccount, type RegisterResult } from '@/lib/registration';

export default function AdminStaffForm({ onSuccess, turnstileToken, turnstileRequired, turnstileWidget }: { onSuccess: (result: RegisterResult) => void; turnstileToken?: string | null; turnstileRequired?: boolean; turnstileWidget?: ReactNode }) {
  ...
  const [f, setF] = useState({
    ...
    password: '', confirm: '', staffId: '', department: '', jobTitle: '', branch: PREFERRED_BRANCHES[0],
  });
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <Field label="Department / Unit" required><TextInput value={f.department} onChange={(v) => set('department', v)} required /></Field>
      <Field label="Job Title" required><TextInput value={f.jobTitle} onChange={(v) => set('jobTitle', v)} required /></Field>
