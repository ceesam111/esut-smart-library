...
const MATRIC_REGEX = /^ESUT\/\d{4}\/\d{5}$/;
...
export default function PatronsImport() {
  ...
  const [skipInvalidMatric, setSkipInvalidMatric] = useState(false);
  ...
  const hasInvalidMatric = (row: ParsedRow): boolean => {
    const matric = getMappedValue(row, 'matric_no');
    return !!(matric && !MATRIC_REGEX.test(matric));
  };

  const invalidMatricCount = csvData.filter(hasInvalidMatric).length;
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      try {
        ...
        if (matricNo && !MATRIC_REGEX.test(matricNo)) {
          if (skipInvalidMatric) {
            result.failed++;
            result.errors.push({ row: rowNum, email, error: `Invalid matric format: ${matricNo}` });
            continue;
          }
        }
  ...
  const downloadTemplate = () => {
    const headers = ['name', 'email', 'matric_no', 'staff_id', 'faculty', 'department', 'level', 'category', 'status'];
    const exampleRows = [
      ['John Doe', 'john.doe@esut.edu.ng', 'ESUT/2024/00142', '', 'Faculty of Education', 'Science Education', '300L', 'Undergraduate', 'active'],
      ['Dr. Jane Smith', 'jane.smith@esut.edu.ng', '', 'ESUT/STAFF/001', 'Faculty of Education', 'Mathematics', '', 'Academic Staff', 'active'],
    ];
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-neutral-600">
              ...
              <div>
                ...
                <ul className="space-y-0.5">
                  ...
                  <li>• <span className="font-mono">department, level, category, status</span></li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="card p-6 space-y-6">
            ...
            <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-300 rounded-xl">
              ...
              <div className="flex-1">
                <p className="font-semibold text-amber-800">
                  {invalidMatricCount} row{invalidMatricCount !== 1 ? 's have' : ' has'} an invalid matric number format
                </p>
                <p className="text-sm text-amber-700 mt-0.5">
                  Expected format: <span className="font-mono">ESUT/YYYY/NNNNN</span> (e.g. ESUT/2024/00142). Rows highlighted in red below.
                </p>
                <label className="flex items-center gap-2 mt-2 text-sm text-amber-800 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={skipInvalidMatric}
                    onChange={e => setSkipInvalidMatric(e.target.checked)}
                    className="w-4 h-4 rounded"
                  />
                  Skip invalid rows during import (import {csvData.length - invalidMatricCount} valid rows only)
                </label>
              </div>
            </div>
          )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {csvData.slice(0, 10).map((row, idx) => {
                  const invalid = hasInvalidMatric(row);
                  return (
                    <tr key={idx} className={`border-b ${invalid ? 'bg-red-50' : 'hover:bg-neutral-50'}`}>
                      {mapping.filter(m => m.dbColumn).map(m => {
                        const val = row[m.csvColumn];
                        const isMatricCol = m.dbColumn === 'matric_no';
                        const isInvalidCell = isMatricCol && val && !MATRIC_REGEX.test(val);
                        return (
                          <td key={m.dbColumn} className={`p-3 ${isInvalidCell ? 'text-red-700 font-semibold' : 'text-neutral-700'}`}>
                            {val || <span className="text-neutral-300">—</span>}
                            {isInvalidCell && <span className="ml-1 text-xs text-red-500">(invalid format)</span>}
                          </td>
                        );
                      })}
                    </tr>
          ...
          <div className="flex items-center justify-between p-4 bg-success-50 rounded-xl border border-success-200">
            <div>
              <p className="font-medium text-success-800">
                {skipInvalidMatric ? csvData.length - invalidMatricCount : csvData.length} patrons ready to import
              </p>
              <p className="text-sm text-success-600">
                {sendWelcomeEmails ? 'Welcome emails will be sent' : 'No emails will be sent'}
                {skipInvalidMatric && invalidMatricCount > 0 && ` · ${invalidMatricCount} invalid rows will be skipped`}
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-4 border-t">
            ...
            <button onClick={performImport} className="btn-primary flex-1">
              Import {skipInvalidMatric ? csvData.length - invalidMatricCount : csvData.length} Patrons
            </button>
          </div>
        </div>
      )}
