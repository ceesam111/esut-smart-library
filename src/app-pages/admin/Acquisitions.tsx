...
function numberToWords(n: number): string {
  ...
  if (!Number.isFinite(n) || n <= 0) return 'zero naira only';
  const units = [['billion', 1_000_000_000], ['million', 1_000_000], ['thousand', 1_000], ['', 1]] as const;
  let whole = Math.floor(n);
  const parts: string[] = [];
  for (const [label, value] of units) {
    const count = Math.floor(whole / value);
    if (count) {
      parts.push(`${chunk(count)}${label ? ` ${label}` : ''}`);
      whole %= value;
    }
  }
  return `${parts.join(' ')} naira only`.replace(/\b\w/g, (c) => c.toUpperCase());
}
...
function PurchaseOrdersTab() {
  ...
  const blankForm = {
    ...
    items: [{ title: '', authors: '', isbn: '', format: 'Library Item', quantity: 1, unit_price: 0 }],
  };
  ...
  const addItem = () => setForm((p) => ({
    ...p, items: [...p.items, { title: '', authors: '', isbn: '', format: 'Library Item', quantity: 1, unit_price: 0 }],
  }));
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="label mb-0 font-semibold">Line Items: books, journals, furniture, equipment, subscriptions, repairs, software, and other library purchases</label>
              ...
              <div className="space-y-2">
                {form.items.map((item, i) => (
                  <div key={i} className="grid grid-cols-12 gap-2 items-center bg-neutral-50 p-2 rounded-lg">
                    <div className="col-span-1 text-xs font-semibold text-neutral-500 text-center">{i + 1}</div>
                    <input className="input col-span-4 text-xs py-1.5" placeholder="Name / Description *" value={item.title}
                      onChange={(e) => updateItem(i, { title: e.target.value })} />
                    <input className="input col-span-2 text-xs py-1.5" placeholder="Remarks" value={item.authors}
              ...
              <p className="text-right text-xs text-neutral-500 mt-1">
                In words: {numberToWords(form.items.reduce((s, i) => s + i.quantity * i.unit_price, 0))}
              </p>
            </div>
      ...
      {/* Detail Modal */}
      {detail && (
        <Modal title={`${detail.po_number} — Items`} onClose={() => setDetail(null)} wide>
          <div className="space-y-3">
            <div className="flex justify-end"><button onClick={() => window.print()} className="btn-outline text-xs">Print Selected Items</button></div>
            <table className="w-full text-sm border border-neutral-200">
              <thead className="bg-neutral-50"><tr>{['S/N', 'Name/Description', 'Quantity', 'Unit Cost', 'Total Cost', 'Remarks'].map((h) => <th key={h} className="p-2 border text-left">{h}</th>)}</tr></thead>
              <tbody>{(detail.purchase_order_items ?? []).map((item, index) => <tr key={item.id}><td className="p-2 border">{index + 1}</td><td className="p-2 border">{item.title}</td><td className="p-2 border">{item.quantity}</td><td className="p-2 border">{fmtMoney(item.unit_price, detail.currency)}</td><td className="p-2 border">{fmtMoney(item.quantity * item.unit_price, detail.currency)}</td><td className="p-2 border">{item.authors || item.status}</td></tr>)}</tbody>
            </table>
            <div className="text-right font-semibold">Grand Total: {fmtMoney(poTotal(detail.purchase_order_items ?? []), detail.currency)}</div>
            <div className="text-right text-sm text-neutral-500">In words: {numberToWords(poTotal(detail.purchase_order_items ?? []))}</div>
          </div>
        </Modal>
      )}
    </div>
...
function BudgetTab() {
  ...
  const [form, setForm] = useState(blank);
  const [budgetRows, setBudgetRows] = useState([{ description: '', quantity: 1, unit_cost: 0, remarks: '' }]);
  ...
  const budgetWorksheetTotal = budgetRows.reduce((sum, row) => sum + row.quantity * row.unit_cost, 0);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card p-5 space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <h3 className="font-semibold text-neutral-800">Detailed Budget Worksheet</h3>
            <p className="text-xs text-neutral-500">Use for books, journals, e-resources, equipment, furniture, software, repairs, training, binding, utilities, and other library budget lines.</p>
          </div>
          <div className="flex gap-2"><button onClick={() => setBudgetRows((rows) => [...rows, { description: '', quantity: 1, unit_cost: 0, remarks: '' }])} className="btn-outline text-xs">Add Row</button><button onClick={() => window.print()} className="btn-primary text-xs">Print Worksheet</button></div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm border border-neutral-200">
            <thead className="bg-neutral-50"><tr>{['S/N', 'Name/Description', 'Quantity', 'Unit Cost', 'Total Cost', 'Remarks', ''].map((h) => <th key={h} className="p-2 border text-left">{h}</th>)}</tr></thead>
            <tbody>{budgetRows.map((row, index) => <tr key={index}><td className="p-2 border">{index + 1}</td><td className="p-2 border"><input className="input text-xs" value={row.description} onChange={(e) => setBudgetRows((rows) => rows.map((r, i) => i === index ? { ...r, description: e.target.value } : r))} /></td><td className="p-2 border"><input type="number" min={1} className="input text-xs" value={row.quantity} onChange={(e) => setBudgetRows((rows) => rows.map((r, i) => i === index ? { ...r, quantity: +e.target.value } : r))} /></td><td className="p-2 border"><input type="number" min={0} className="input text-xs" value={row.unit_cost} onChange={(e) => setBudgetRows((rows) => rows.map((r, i) => i === index ? { ...r, unit_cost: +e.target.value } : r))} /></td><td className="p-2 border font-semibold">{fmtMoney(row.quantity * row.unit_cost, 'NGN')}</td><td className="p-2 border"><input className="input text-xs" value={row.remarks} onChange={(e) => setBudgetRows((rows) => rows.map((r, i) => i === index ? { ...r, remarks: e.target.value } : r))} /></td><td className="p-2 border"><button onClick={() => setBudgetRows((rows) => rows.filter((_, i) => i !== index))} className="text-xs text-red-600">Delete</button></td></tr>)}</tbody>
          </table>
        </div>
        <div className="text-right text-sm"><p className="font-semibold">Grand Total: {fmtMoney(budgetWorksheetTotal, 'NGN')}</p><p className="text-neutral-500">In words: {numberToWords(budgetWorksheetTotal)}</p></div>
      </div>
