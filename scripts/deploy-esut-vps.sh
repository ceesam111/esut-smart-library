...
# Turnstile site key (public) lives in the merged extra env file
NEXT_PUBLIC_CLOUDFLARE_SITE_KEY=$(grep '^NEXT_PUBLIC_CLOUDFLARE_SITE_KEY=' /root/esut-extra.env | head -1 | cut -d= -f2- || true)
export NEXT_PUBLIC_APP_URL NEXT_PUBLIC_APP_BASE_URL NEXT_PUBLIC_SUPABASE_URL NEXT_PUBLIC_SUPABASE_ANON_KEY NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY NEXT_PUBLIC_CLOUDFLARE_SITE_KEY
export CACHE_BUST=$(date +%s)
echo "URL=$NEXT_PUBLIC_APP_URL SB=$NEXT_PUBLIC_SUPABASE_URL TURNSTILE=${NEXT_PUBLIC_CLOUDFLARE_SITE_KEY:+set}"
